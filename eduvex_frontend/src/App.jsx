...
import OAuthSuccess from "./pages/user/auth/OAuthSuccess";
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (response.status === 200) {
          // loginSuccess now receives user with onboardingCompleted included
    ...
    const checkAdminAuth = async () => {
      try {
        ...
        if (response.status === 200) {
          dispatch(adminAuthSuccess({ admin: response.data.admin }));
        }
      } catch (error) {
  ...
  return (
    <Router>
      <Routes>
        ...
        <Route path="/complete-profile" element={isUserAuth ? <CompleteProfile /> : <Navigate to="/login" replace />} />

        {/* Tutor application page (for applying / pending / rejected) */}
        <Route
          path="/tutor/application"
          element={isUserAuth ? <Tutordashboardguard /> : <Navigate to="/login" replace />}
        />

        {/* Tutor dashboard — guard handles: onboarding vs dashboard */}
        <Route
          path="/tutor/dashboard"
          element={isUserAuth ? <CourseTable /> : <Navigate to="/login" replace />}
        />
